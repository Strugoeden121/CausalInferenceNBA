...
DAG_DATA_STEPH = [
    ...
    ("player_minutes", "did_steph_scored_more_than_20_points"),
    ...
    ("did_steph_scored_more_than_20_points", "team_points"),     # Points scored influence the winner
    ("did_steph_scored_more_than_20_points", "avg_team_points"),
...
def augment_steph_curry_dataset():
    ...
    # Add a column to indicate if Steph Curry scored more than 20 points
    steph_curry_data['did_steph_scored_more_than_20_points'] = steph_curry_data.apply(
        lambda row: 1 if row['player_name'] == 'Stephen Curry' and row['points'] >= 20 else 0, axis=1)

    print(steph_curry_data['did_steph_scored_more_than_20_points'].value_counts())
...
def steph_causal_model(dag, steph_curry_data):
    ...
    model = CausalModel(
        ...
        treatment='did_steph_scored_more_than_20_points',
        outcome='gsw_won',
        graph=dag
    )
    ...
    if is_treatment_significant(causal_estimate):
        print(f'Treatment: steph_scored_20_plus, Outcome: gsw_won, ATE: {causal_estimate.value}')
    else:
        print(
            f'Treatment: steph_scored_20_plus, Outcome: gsw_won, ATE: Not significant, p-value: {causal_estimate.test_stat_significance()["p_value"]}')


if __name__ == '__main__':
    print("Treatment - Steph Curry scoring more then 20 points in game")
