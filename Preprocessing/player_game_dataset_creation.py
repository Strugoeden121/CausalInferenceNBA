...
def calculate_avg_team_stats(game_player_stats):
    ...
    game_team_averages = filtered_games_scoring.groupby(['game_date', 'player_team']).agg({
        'points': 'mean', 'rebounds': 'mean', 'assists': 'mean', 'minutes': 'mean'}).reset_index()

    game_team_averages.rename(columns={'points': 'avg_team_points','rebounds': 'avg_team_rebounds',
        'assists': 'avg_team_assists', 'minutes': 'avg_team_minutes'}, inplace=True)
...
def add_rival_team_stats(game_player_stats):
    ...
    rival_game_team_averages.rename(columns={ 'points': 'avg_rival_team_points','rebounds': 'avg_rival_team_rebounds',
        'assists': 'avg_rival_team_assists','minutes': 'avg_rival_team_minutes'}, inplace=True)
...
if __name__ == "__main__":
    ...
    game_player_stats2.to_csv(f"{BASE_DIR}/game_player_stats_first.csv", index=False)
    game_player_stats_final = add_rival_team_stats(game_player_stats2)
    game_player_stats_final.to_csv(f"{BASE_DIR}/game_player_stats_final.csv", index=False)
